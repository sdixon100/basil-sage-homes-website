...
# Creates a complete PEM certificate bundle for basilandsagehomes.com
...
Write-Host "  Root CA Certificate created: root-ca.crt" -ForegroundColor Green
...
Write-Host "  Intermediate CA Certificate created: intermediate-ca.crt" -ForegroundColor Green
...
& $opensslPath x509 -req -in "$outputDir\server.csr" `
    ...
    -extfile "$outputDir\openssl.cnf" 2>&1 | Out-Null
Write-Host "  Server Certificate created: server.crt" -ForegroundColor Green

# Create PEM formatted certificate bundles
Write-Host "`nCreating PEM Certificate Bundles..." -ForegroundColor Yellow

$serverKey = Get-Content "$outputDir\server.key" -Raw
...
Set-Content -Path "$outputDir\server-combined.pem" -Value ($serverKey + "`n" + $serverCert) -NoNewline
Write-Host "  server-combined.pem (Private Key + Server Certificate)" -ForegroundColor Green

# 2. Full chain certificate (PEM format)
Set-Content -Path "$outputDir\fullchain.pem" -Value ($serverCert + "`n" + $intermediateCert + "`n" + $rootCert) -NoNewline
Write-Host "  fullchain.pem (Server + Intermediate + Root)" -ForegroundColor Green

# 3. Chain certificate (PEM format)
Set-Content -Path "$outputDir\chain.pem" -Value ($serverCert + "`n" + $intermediateCert) -NoNewline
Write-Host "  chain.pem (Server + Intermediate)" -ForegroundColor Green

# 4. Complete PEM bundle (key + full chain)
Set-Content -Path "$outputDir\complete.pem" -Value ($serverKey + "`n" + $serverCert + "`n" + $intermediateCert + "`n" + $rootCert) -NoNewline
Write-Host "  complete.pem (Private Key + Full Certificate Chain)" -ForegroundColor Green

# Also create .crt versions for compatibility
Set-Content -Path "$outputDir\chain.crt" -Value ($serverCert + "`n" + $intermediateCert) -NoNewline
Set-Content -Path "$outputDir\fullchain.crt" -Value ($serverCert + "`n" + $intermediateCert + "`n" + $rootCert) -NoNewline
...
Write-Host "  complete.pem         - Private Key + Full Chain (all-in-one)" -ForegroundColor Cyan
Write-Host "  server-combined.pem  - Private Key + Server Certificate" -ForegroundColor Cyan
Write-Host "  fullchain.pem        - Server + Intermediate + Root Certificates" -ForegroundColor Cyan
Write-Host "  chain.pem            - Server + Intermediate Certificates" -ForegroundColor Cyan

Write-Host "`nIndividual Certificate Files:" -ForegroundColor White
Write-Host "  server.key           - Server Private Key (KEEP SECURE!)" -ForegroundColor Yellow
Write-Host "  server.crt           - Server Certificate" -ForegroundColor Cyan
Write-Host "  intermediate-ca.crt  - Intermediate CA Certificate" -ForegroundColor Cyan
Write-Host "  root-ca.crt          - Root CA Certificate" -ForegroundColor Cyan
...
Write-Host "  For most web servers: Use 'server.key' + 'chain.pem'"
Write-Host "  For all-in-one setup: Use 'complete.pem'"
Write-Host "  For Nginx/Apache: Use 'server.key' + 'fullchain.pem'"
